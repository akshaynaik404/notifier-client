...
$(function () {
	...
	vars.$personalSignupContainer.find(".personal-signup-submit-btn").click(
		function (e) {
			e.preventDefault();
			let inputIds = ["#mobile_no", "#new-notifier-id", "#new-password",
				"#confirm-password"];
...
function signUp(inputIds, containerSelector, dataEndpointUrl, $clickedBtn) {
	...
	let dataObj = (function (ids) {
		...
		ids.forEach(function (id) {
			dataObj[id.slice(1, id.length)] = functions.getValue(id,
				containerSelector);
		});
		return dataObj;
	})(inputIds);
